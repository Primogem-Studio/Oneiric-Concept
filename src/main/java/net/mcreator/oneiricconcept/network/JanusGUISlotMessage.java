...
public record JanusGUISlotMessage(int slotID, int x, int y, int z, int changeType, int meta) implements CustomPacketPayload {
	...
	public static void handleData(final JanusGUISlotMessage message, final IPayloadContext context) {
	...
	public static void handleSlotAction(Player entity, int slot, int changeType, int meta, int x, int y, int z) {
		...
		if (slot == 0 && changeType == 0) {
			...
			JanusDataProcedure.execute(world, x, y, z);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		OneiricconceptMod.addNetworkMessage(JanusGUISlotMessage.TYPE, JanusGUISlotMessage.STREAM_CODEC, JanusGUISlotMessage::handleData);
	}
}