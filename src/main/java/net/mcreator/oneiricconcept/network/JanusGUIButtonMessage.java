...
public record JanusGUIButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static void handleData(final JanusGUIButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		if (buttonID == 0) {
			...
			JanusHiddenPassageProcedure.execute(world, x, y, z, entity);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		OneiricconceptMod.addNetworkMessage(JanusGUIButtonMessage.TYPE, JanusGUIButtonMessage.STREAM_CODEC, JanusGUIButtonMessage::handleData);
	}
}