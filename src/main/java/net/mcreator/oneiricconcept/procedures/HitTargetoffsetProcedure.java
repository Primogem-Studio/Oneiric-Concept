		...
		if (sourceentity instanceof LivingEntity _livEnt2 && _livEnt2.hasEffect(OneiricconceptModMobEffects.TARGETOFFSET) && RandomProcedure.execute(world, (efflevel + 1) * 0.01)) {
			...
			if (world.getLevelData().getGameRules().getBoolean(OneiricconceptModGameRules.OCDEBUG)) {
				if (world instanceof ServerLevel _level) {
					_level.getServer().getPlayerList().broadcastSystemMessage(Component.literal(("\u00A7" + ("1234569abcde".substring((int) (world.dayTime() % 12))).substring(0, 0) + Calendar.getInstance().getTime().toString())), false);
				}
			}
