			...
			if (0 < EquilibriumLevel) {
				for (int index0 = 0; index0 < (int) EquilibriumLevel; index0++) {
					if (RandomProcedure.execute(world, 0.2 * EquilibriumLevel)) {
						itmsstk = (RandomProcedure.execute(world, 0.1 * EquilibriumLevel) ? new ItemStack(OneiricconceptModItems.CONQUEROR_S_WILL.get()) : new ItemStack(OneiricconceptModItems.USURPER_S_SCHEME.get())).copy();
					} else {
						itmsstk = new ItemStack(OneiricconceptModItems.THIEF_S_INSTINCT.get()).copy();
					}
