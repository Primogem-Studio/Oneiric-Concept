			...
			for (int index1 = 0; index1 < (int) range; index1++) {
				...
				for (int index2 = 0; index2 < (int) range; index2++) {
					...
					if (RandomProcedure.execute(world, probability)) {
						world.setBlock(BlockPos.containing(x + sx, y + sy, z + sz), BuiltInRegistries.BLOCK.get(ResourceLocation.parse("primogemcraft:yuanshikuangshi")).defaultBlockState(), 3);
					}
					sz = sz + 1;
				}
				sy = sy + 1;
